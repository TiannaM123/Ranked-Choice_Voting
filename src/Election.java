...
public class Election {
    ...
    // A string array to hold the name of the winners
    private List<String> winnerName = new ArrayList<>();
    private List<String> eliminatedName = new ArrayList<>();


    //Ballot list that holds the eliminated ballots to use in event of tie
    private List<Ballot> eliminatedBallots = new ArrayList<>();
    ...
    /**
     * Finds out what is the highest number of votes, then adds the
     *people with the most number of votes to list winnerName to
     *show winners
     * @return If there is a winner, this method returns a list containing just
     * the winner's name is returned.  If there is a tie, this method returns a
     * list containing the names of the tied candidates.
     */
    public List<String> mostVotes(){
      //this list helps keep track of eliminated candidate's ballots
      List<Ballot> deletedBallot;
      int max = 0;
      ...
      for(int i = 0; i < candidates.length; i++){
        if(candidates[i].getVotes() == max){
          winnerName.add(candidates[i].getName());
        }
        else{
          deletedBallot = candidates[i].eliminate();
    ...
    //     System.out.println(deletedBallot.get);
    //     //deletedBallot.getTopCandidate();
    //   }
      // int elimiatedPosition;
      // boolean isPresent;
      // List<Ballot> delBallot;
      // for(int i = 0; i < candidates.length; i++){
      //   isPresent = false;
      //   for(int j = 0; j < winnerName.size(); j++){
      //     if(candidates[i].getName() == winnerName[j]){
      //       isPresent = true;
      //     }
      //     if(isPresent){
      //       delBallot = candidates[i].eliminate();
      //       elimiatedPosition = delBallot.getTopCandidate();
      //       delBallot.eliminateCandidate(elimiatedPosition);
      //     }
      //   }
      // }
